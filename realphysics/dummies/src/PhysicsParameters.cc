...
const PhysicsParameters*  PhysicsParameters::GetPhysicsParametersForRegion(int regionindx) {
  ...
  for (size_t i=0; i<gThePhysicsParametersTable.size(); ++i) {
...
void PhysicsParameters::SetNumLossTableBins(int val) {
  if (val>4 && val<10000000) {
    ...
    std::cerr << "  **** WARNING: PhysicsParameters::SetNumLossTableBins() " << std::endl
              << "    Value of fNumLossTableBins is out of range: "   << val << " so it's ignored!"
              << std::endl;
  }
}
...
void PhysicsParameters::SetNumLossTableBinsPerDecade(int val) {
  if (val>4 && val<1000000) {
    ...
    std::cerr << "  **** WARNING: PhysicsParameters::SetNumLossTableBinsPerDecade() " << std::endl
              << "    Value of fNumLossTableBinsPerDecade is out of range: "   << val << " so it's ignored!"
              << std::endl;
  }
}
...
void PhysicsParameters::SetNumLambdaTableBins(int val) {
  if (val>4 && val<10000000) {
    ...
    fNumLambdaTableBinsPerDecade = std::lrint(fNumLambdaTableBins/std::log10(fMaxLambdaTableEnergy/fMinLambdaTableEnergy));
  } else {
    std::cerr << "  **** WARNING: PhysicsParameters::SetNumLambdaTableBins() " << std::endl
              << "    Value of fNumLambdaTableBins is out of range: "   << val << " so it's ignored!"
              << std::endl;
  }
}
...
void PhysicsParameters::SetLinearEnergyLossLimit(double val) {
  if (val>0.0 && val<0.5) {
    ...
    std::cerr << "  **** WARNING: PhysicsParameters::SetLinearEnergyLossLimit " << std::endl
              << "    Value of fLinearEnergyLossLimit is out of range: " << val << " so it's ignored!"
              << std::endl;
  }
}
...
//  using geant::eV;
  ...
  flux << std::setw(100) << std::setfill('=') << std::left << "  =" << std::endl
       << std::setw(40) << std::setfill('-') << std::left << "  -" << " Physics parameters "
       << std::setw(40) << std::setfill('-') << std::left << "-"   << std::endl
       << std::setw(100) << std::setfill('=') << std::left << "  ="
  ...
  flux << "    **** (Static)Parameters that must have the same value in each region: \n"
       ...
       << physpar.GetMinAllowedGammaCutEnergy()/GeV    << " [GeV]\n"
       << "     - fMaxAllowedGammaCutEnergy    =   " << std::setw(10)
       << physpar.GetMaxAllowedGammaCutEnergy()/GeV    << " [GeV]\n"
       << "     - fMinAllowedElectronCutEnergy =   " << std::setw(10)
       << physpar.GetMinAllowedElectronCutEnergy()/GeV << " [GeV]\n"
       << "     - fMaxAllowedElectronCutEnergy =   " << std::setw(10)
       << physpar.GetMaxAllowedElectronCutEnergy()/GeV << " [GeV]\n"
       << "     - fMinAllowedPositronCutEnergy =   " << std::setw(10)
       << physpar.GetMinAllowedPositronCutEnergy()/GeV  << " [GeV]\n"
       << "     - fMaxAllowedPositronCutEnergy =   " << std::setw(10)
       << physpar.GetMaxAllowedPositronCutEnergy()/GeV  << " [GeV]\n"
       << "     - fDefaultGammaCutInLength     =   " << std::setw(10)
       << physpar.GetDefaultGammaCutInLength()/cm       << "  [cm]\n"
       << "     - fDefaultElectronCutInLength  =   " << std::setw(10)
       << physpar.GetDefaultElectronCutInLength()/cm    << "  [cm]\n"
       << "     - fDefaultPositronCutInLength  =   " << std::setw(10)
       << physpar.GetDefaultPositronCutInLength()/cm    << "  [cm]\n"
       << "     - fDefaultGammaCutInEnergy     =   " << std::setw(10)
       << physpar.GetDefaultGammaCutInEnergy()/GeV      << " [GeV]\n"
       << "     - fDefaultElectronCutInEnergy  =   " << std::setw(10)
       << physpar.GetDefaultElectronCutInEnergy()/GeV    << " [GeV]\n"
       << "     - fDefaultPositronCutInEnergy  =   " << std::setw(10)
       << physpar.GetDefaultPositronCutInEnergy()/GeV    << " [GeV]\n\n";

  flux << "    **** Active in regions: ";
       for (size_t i=0; i<physpar.GetListActiveRegions().size(); ++i) {
  ...
  flux << std::left  << std::setw(textW) << "     - Minimum kinetic energy of the lambda table grid "
       ...
       << std::right << std::setw(valW)  << physpar.GetNumLambdaTableBinsPerDecade() << "\n"
       << std::endl;
   ...
   flux << std::setw(90) << std::left << std::setfill('-')
        << "     --- Other parameters " << std::setfill(' ') << "\n";
   flux << std::left  << std::setw(textW) << "     - Compute CSDA range tables  "
        << std::right << std::setw(valW)  << physpar.GetIsComputeCSDARange() << "\n"
        << std::left  << std::setw(textW) << "     - Linear energy loss approximation limit  "
        << std::right << std::setw(valW)  << physpar.GetLinearEnergyLossLimit() << "\n"
