...
TVNudyModel::TVNudyModel(TGeoElementRN *mat, Reaction_t reac, unsigned long temp, TParticlePDG *projectile,
    ...
    : TNamed(TNudyCore::Instance()->GetKey(mat, reac, temp), ""), fMAT(0), fTemp(temp), fEndf(mat->ENDFCode()),
      fPdg(projectile->PdgCode()), fMaterial(mat), fReaction((Reaction_t)reac), fProjectile(projectile),
      fEXSect_length(0), fE_file3(NULL), fXSect_file3(NULL), f4nens(0), f4eins(0), fAPAlias(NULL), f4Tein(0), f4Tel(0),
      nens(0), nperc(25), maxpop(50), fEPtable(NULL), fPerc(NULL), f5Tein(-1), f5Tel(0), fEPAlias(NULL)
{
      ...
      for (int jg = 1; jg <= nens; jg++) {
        ...
        for (int jef = 1; jef <= nEout[jg - 1]; jef++) {
          ...
          if (!(efen >= (energy - u))) {
            ...
            if (jef > 1) hint += 0.5 * (efen - eold) * (prob + pold);
            eold = efen;
            pold = prob;
          } else {
            break;
...
void TVNudyModel::PopulateGrid(int index)
{
  ...
  double ef         = emin / fact;
  if (index > nEout.GetSize() - 1) nEout.Set(index + 1);
  nEout[index] = max;
