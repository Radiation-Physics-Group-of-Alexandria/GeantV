...
TNudyEndfEnergyAng::TNudyEndfEnergyAng(TNudyEndfFile *file, double iQValue[])
{
  ...
  while ((sec = (TNudyEndfSec *)secIter.Next())) {
    ...
    for (int k = 0; k < NK; k++) {
     // std::cout<<k<<" NK "<<NK<< std::endl;
      ...
      // int LIP =tab1->GetL1();
      int LAW = tab1->GetL2();
      //std::cout<<MT<<"  B4Law: "<<LAW<<std::endl;
      if (LAW == 3 || LAW == 4 || LAW == 0) continue;
      //std::cout<<MT<<"  After Law: "<<LAW<<std::endl;
      law.push_back(LAW);
      NR            = tab1->GetN1();
      NP            = tab1->GetN2();
      divr          = div(ZAP, 1000);
	...
	MtNumNeutron.push_back (MT) ;
      } else if (particleZ == 0 && particleA == 0) {
	MtNumPhoton.push_back (MT) ;
      } else if (particleZ != 0 && particleA >= 1) {
	MtNumCharge.push_back (MT) ;
      }
      //std::cout<<"LAW = "<< LAW <<" MT "<<MT <<" NP "<< NP <<" ZAP = "<< ZAP <<" AWP "<<AWP <<" parZ "<< particleZ
      ...
      if (LAW == 2) {
        ...
        for (int lis = 0; lis < np2; lis++) {
         ...
         // std::cout<<"get energy from header: "<<header->GetC2()<<std::endl;
          LANG = header->GetL1();
          // NW   = header->GetN1();
          NL = header->GetN2();
          NP = NL;
      ...
      case 1: {
        ...
        for (int crs = 0; crs < NP; crs++) {
          ...
          //if(MT==16){std::cout<<"tab1->GetX(crs)-->"<<tab1->GetX(crs)<<"\t"<<"tab1->GetY(crs)-->"<<tab1->GetY(crs)<<std::endl;}
        }
        TNudyEndfTab2 *tab2 = (TNudyEndfTab2 *)recIter.Next();
        LANG                = tab2->GetL1();
        // LEP  = tab2->GetL2();
        nr2 = tab2->GetN1();
        np2 = tab2->GetN2();
        //std::cout<<"MT  "<<MT<<" LANG "<<LANG<<std::endl;
        //std::cout<<"LANG "<< LANG <<"  "<< NA<<"  "<< MT<< std::endl;
        for (int lis = 0; lis < np2; lis++) {
          TNudyEndfList *header = (TNudyEndfList *)recIter.Next();
          ein.push_back(header->GetC2());
         // std::cout<<"get energy from header:law1  "<<header->GetC2()<<"  "<<MT<<std::endl;
          double sumein = 0;
          // ND   = header->GetL1();
          NA  = header->GetL2();
          NEP = header->GetN2();
          ...
          if (LANG == 2 && NA < 2) {
            ...
            if( particleZ == 0 && particleA == 1 ){
              Mn1 = 1;   
              mn  = 0.5; 
	     }else if(particleZ == 1 && particleA == 1){
	      Mn1 = 1;   
              mn  = 1;
	     }else if(particleZ == 1 && particleA == 2){
	      Mn1 = 1;   
              mn  = 1;
	     }else if(particleZ == 2 && particleA == 4){
	      Mn1 = 0;   
              mn  = 2;
	     }else{
	      Mn1 = 0.0;
	      mn = 0.0;
	     //std::cout<<"About other particles, values are not avaialable, see endf manual"<<std::endl;
	     }
             int k1 = 0;
             do {
              double x = -1. + k1 * 0.02;
          ...
          // if(MT==28)std::cout<<"LANG::::"<<LANG<<"  NA---->"<<NA<<std::endl;
            for (int lis1 = 0; lis1 < NEP; lis1++) {
                 edes6.push_back(header->GetLIST(lis1 * (NA + 2) + 0));
              //if(MT==28)std::cout<<"edes6: "<<" NA-->"<<NA<<" NEP:"<<NEP<<"\t"<<
              //                      header->GetLIST(lis1 * (NA + 2) + 0)<<std::endl;
              for (int j = 0; j < NA + 1; j++) {
                lCoef1.push_back(header->GetLIST(lis1 * (NA + 2) + j + 1));
                 //std::cout <<"lCoef------>"<<header->GetLIST(lis1*(NA+2) + j + 1) << std::endl;
              }
              lCoef.push_back(lCoef1);
              lCoef1.clear();
            }
            ...
            do {
              ...
              for (unsigned long m = 0; m < edes6.size(); m++) {
                ...
                for (unsigned long j = 0; j < lCoef[m].size(); j++) {
                  double leg = ROOT::Math::legendre(j+1, x);
                  fme += 0.5 * (2. * (j+1) + 1.) * lCoef[m][j] * leg;
                  // std::cout << x <<"  "<< leg<<" leg "<<lCoef[m][j] <<"  "<<lCoef[m].size() <<"  "<< fme << std::endl;
                }
                 if(fme > 1E-15){
              ...
              for (unsigned long i = 0; i < energyFile5.size(); i++) {
                ...
                pdfe.push_back(energyPdfFile5[i]);
                cdfe.push_back(energyCdfFile5[i]);
                // std::cout <<sumprob<<" energy "<< energyFile5[i] << "  "<< energyPdfFile5[i] <<"  "<<
                // energyCdfFile5[i] << std::endl;
              }
              eout2de.push_back(eoute);
              pdf2de.push_back(pdfe);
              cdf2de.push_back(cdfe);
	...
	LANG = header->GetL1();
      ...
      case 6: {
        ...
        do {
          ...
          // std::cout << energy <<"  "<< fE1[NP-1] <<"  "<< EiMax <<"  \t"<< AWP << std::endl;
		...
		rn34 		= rn3 * rn3 + rn4 * rn4;
            ...
            for (unsigned long i = 0; i < energyFile5.size(); i++) {
              ...
              pdfe.push_back(energyPdfFile5[i]);
              cdfe.push_back(energyCdfFile5[i]);
              // std::cout<< energyFile5[i] << "  "<< energyPdfFile5[i] << "  "<< energyCdfFile5[i] <<"  "<< cosCM << std::endl;
            }
            eout2de.push_back(eoute);
            pdf2de.push_back(pdfe);
            cdf2de.push_back(cdfe);
  ...
  Mt6Neutron.push_back (MtNumNeutron);
  ...
  //  std::cout<<"MT = "<< Mt6Values[0][i]<<"  "<<Mt6Values[0].size()<<"  "<< Law6[0][i] <<"  "<< ZD6[0][i] <<"  "<<
  //  AD6[0][i] << std::endl;
  //  }
...
//------------------------------------------------------------------------------------------------------
TNudyEndfEnergyAng::InfoProductsFile6 TNudyEndfEnergyAng::GetInformation(){
...
{
  if (Mt6Neutron.size() <= 0) return -1;
  int size = Mt6Neutron[ielemId].size();
  for (int i = 0; i < size; i++) {
    if (Mt6Neutron[ielemId][i] == mt) {
      return Mt6Neutron[ielemId][i];
    }
  }
  return -1;
}
...
int TNudyEndfEnergyAng::GetZd6(int ielemId, int mt)
{
  ...
  for (int i = 0; i < size; i++) {
    if (Mt6Values[ielemId][i] == mt) {
    //std::cout<<"zd6: "<<ZD6[ielemId][i]<<std::endl;
...
int TNudyEndfEnergyAng::GetLaw6(int ielemId, int mt)
{
  ...
  //std::cout << "GetLaw6(" << ielemId << ", " << mt << ") -- size: " << size << std::endl; 
  //if(mt==60)std::cout<<"TNudyEndfEnergyAng::GetLaw6: law size -> "<<size<<std::endl;
  for (int i = 0; i < size; i++) {
   // std::cout << "Mt6Values[ielemId][" << i << "] :" <<  Mt6Values[ielemId][i] << std::endl;
    if (Mt6Values[ielemId][i] == mt) {
     //std::cout<<mt<<"     TNudyEndfEnergyAng::GetLaw6: law -> "<<Law6[ielemId][i]<<std::endl;
...
{
//std::cout<<"TNudyEndfEnergyAng::GetCos64"<<std::endl;
  fRnd  = new TRandom3(0);
  int i = -1;
//   std::cout <<"mt4 values in file 6  "<< Mt4Values[ielemId].size() << std::endl;
  ...
  for (int j = 0; j < size; j++) {
    ...
    // std::endl;
    if (rnd2 <= cosCdf4OfMts[ielemId][i][min][j]) {
      k                    = j - 1;
...
//    for(int j = 0; j < size; j++){
//      std::cout << energy4OfMts[ielemId][i][min]<< "  "<< Mt4Values[ielemId][i]
//      <<"  "<< cos4OfMts[ielemId][i][min][j] <<"  "<<cosPdf4OfMts[ielemId][i][min][j] 
//      <<"  "<<cosCdf4OfMts[ielemId][i][min][j] << std::endl;
//    }
  ...
  double plsq = plk2 + 2 * plk * (rnd2 - cosCdf4OfMts[ielemId][i][min][k]);
  double Ang  = 0;
  if (plk != 0 && plsq > 0) {
    Ang = cos4OfMts[ielemId][i][min][k] + (sqrt(plsq) - cosPdf4OfMts[ielemId][i][min][k]) / plk;
  } else {
...
double TNudyEndfEnergyAng::GetCos6(int ielemId, int mt, double energyK)
{
  ...
  //std::cout<<"mt "<< mt <<" "<< i << "  "<< energy5OfMts[ielemId][i].size()<<"  "<<energy5OfMts[ielemId][i][max] << std::endl;
  int mid = 0;
  ...
  if (plk != 0 && plsq > 0) {
    Ang = cos6OfMts[ielemId][i][min][k] + (sqrt(std::fabs(plsq)) - cosin6Pdf[ielemId][i][min][k]) / plk;
     //std::cout<< Ang <<" first "<< rnd1 << std::endl;
  } else {
    Ang = 2 * rnd1 - 1;
    //std::cout<< Ang <<" sec  "<< rnd1 << std::endl;
  }
  // std::cout<<"B4 return "<< Ang << std::endl;
  return Ang;
}
//------------------------------------------------------------------------------------------------------
double TNudyEndfEnergyAng::GetEnergy6(int ielemId, int mt, double energyK)
{
  fRnd  = new TRandom3(0);
  int i = -1;
//    std::cout <<"mt5 values in file 6  "<< Mt5Values[ielemId].size() <<"  "<< ZD6[ielemId][0] <<"  "<< ZD6[ielemId][1] << std::endl;
 for (unsigned int l = 0; l < Mt5Values[ielemId].size(); l++) {
    if (Mt5Values[ielemId][l] == mt) {
      i = l;
      break;
  ...
  double edes = 0;
  if (plk != 0)
    edes = energyOut6OfMts[ielemId][i][min][k][m] +
           (sqrt(plk2 + 2 * plk * (rnd1 - energyCdf6OfMts[ielemId][i][min][k][m])) -
            energyPdf6OfMts[ielemId][i][min][k][m]) /
               plk;
               
       //std::cout<<"edes: "<<edes<<std::endl;        
               
  return edes;
}
//______________________________________________________________________________
void TNudyEndfEnergyAng::GetAd6V(int ielemId, int mt)
{
  int size = AD6[ielemId].size();
  for (int i = 0; i < size; i++) {
    if (Mt6Values[ielemId][i] == mt) {
       infoParticles.pA.push_back(AD6[ielemId][i]);
    }
  }
}
//______________________________________________________________________________
void TNudyEndfEnergyAng::GetZd6V(int ielemId, int mt)
{
  int size = ZD6[ielemId].size();
  int count=0;
  for (int i = 0; i < size; i++) {
    if (Mt6Values[ielemId][i] == mt) {
    count=count+1;
    // std::cout<<"No. of particles: TNudyEndfEnergyAng::GetZd6V "<<count<<std::endl;
      //std::cout<<"zd6: "<<ZD6[ielemId][i]<<std::endl;
      infoParticles.pZ.push_back(ZD6[ielemId][i]);
    }
  }
}
//SPB
...
//______________________________________________________________________________
//------------------------------------------------------------------------------------------------------
void TNudyEndfEnergyAng::GetLaw6V(int ielemId, int mt)
{
   int size = Law6[ielemId].size();
  //std::cout<<" TNudyEndfEnergyAng::GetLaw6V -> size-> "<< size<<std::endl;
  for (int i = 0; i < size; i++) {
    if (Mt6Values[ielemId][i] == mt) {
    //std::cout<<"Law6[ielemId][i]: "<<Law6[ielemId][i]<<std::endl;
       infoParticles.lawZA.push_back(Law6[ielemId][i]);
    }
  }
}
//------------------------------------------------------------------------------------------------------
void TNudyEndfEnergyAng::GetCos64V(int ielemId, int mt, double energyK)
{
  fRnd  = new TRandom3(0);
  int i = -1;
  for (unsigned int l = 0; l < Mt4Values[ielemId].size(); l++) {
  ...
  //std::cout <<Ang<<"  uniform angle "<< Ang << std::endl;
  infoParticles.cosang64.push_back(Ang);
...
//------------------------------------------------------------------------------------------------------
void TNudyEndfEnergyAng::GetCos6V(int ielemId, int mt, double energyK)
{
  fRnd  = new TRandom3(0);
  int i = -1;
  for (unsigned int l = 0; l < Mt5Values[ielemId].size(); l++) {
 ...
 // }
  ...
  infoParticles.cosang6.push_back(Ang);
  
    }
  }
}
//------------------------------------------------------------------------------------------------------
void TNudyEndfEnergyAng::GetEnergy6V(int ielemId, int mt, double energyK)
{
  fRnd  = new TRandom3(0);
  int i = -1;
  for (unsigned int l = 0; l < Mt5Values[ielemId].size(); l++) {
  ...
  if (plk != 0)
    edes = energyOut6OfMts[ielemId][i][min][k][m] +
           (sqrt(plk2 + 2 * plk * (rnd1 - energyCdf6OfMts[ielemId][i][min][k][m])) -
            energyPdf6OfMts[ielemId][i][min][k][m]) /
               plk;
    infoParticles.Energy6.push_back(edes);       
    }
  }         
}

//------------------------------------------------------------------------------------------------------
void TNudyEndfEnergyAng::Reset() {
  infoParticles.pZ.clear();
  infoParticles.pA.clear();
  infoParticles.lawZA.clear();
  infoParticles.cosang6.clear();
  infoParticles.cosang64.clear();
  infoParticles.Energy6.clear();
 }
