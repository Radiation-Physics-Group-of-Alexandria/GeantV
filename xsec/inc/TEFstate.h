...
bool CheckAlign() {
  bool isaligned=true;
  if(((unsigned long) fEGrid) % sizeof(fEGrid[0]) != 0) {printf("TEFstate::fEGrid misaligned\n");isaligned=false;}
  if(((unsigned long) &fAtcm3) % sizeof(fAtcm3) != 0) {printf("TEFstate::fAtcm3 misaligned\n");isaligned=false;}
  if(((unsigned long) &fEmin) % sizeof(fEmin) != 0) {printf("TEFstate::fEmin misaligned\n");isaligned=false;}
  if(((unsigned long) &fEmax) % sizeof(fEmax) != 0) {printf("TEFstate::fEmax misaligned\n");isaligned=false;}
  if(((unsigned long) &fEilDelta) % sizeof(fEilDelta) != 0) {printf("TEFstate::fEilDelta misaligned\n");isaligned=false;}
  if(((unsigned long) &fDens) % sizeof(fDens) != 0) {printf("TEFstate::fDens misaligned\n");isaligned=false;}
  if(((unsigned long) &fEle) % sizeof(fEle) != 0) {printf("TEFstate::fEle misaligned\n");isaligned=false;}
  if(((unsigned long) &fNEbins) % sizeof(fNEbins) != 0) {printf("TEFstate::fNEbins misaligned\n");isaligned=false;}
  if(((unsigned long) &fNEFstat) % sizeof(fNEFstat) != 0) {printf("TEFstate::fNEFstat misaligned\n");isaligned=false;}
  if(((unsigned long) &fNRpart) % sizeof(fNRpart) != 0) {printf("TEFstate::fNRpart misaligned\n");isaligned=false;}
  for(auto i=0; i< fNRpart; ++i)
    if(((unsigned long) fPFstateP[i]) % sizeof(double) != 0) {printf("TEFstate::fPFstateP[%d misaligned\n",i);isaligned=false;}
#ifdef MAGIC_DEBUG
  if(((unsigned long) &fMagic) % sizeof(fMagic) != 0) {printf("TEFstate::fMagic misaligned\n");isaligned=false;}
#endif
  if(((unsigned long) &fStore) % sizeof(double) != 0) {printf("TEFstate::fStore misaligned\n");isaligned=false;}
  return isaligned;
}
