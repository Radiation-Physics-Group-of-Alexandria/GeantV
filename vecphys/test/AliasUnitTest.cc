...
void interactAlias(GUAliasTable* fAliasTable, TH1F *histo, int size, double lowEdge, double upEdge){
    ...
    double chi2=histo->Chi2Test(reconstructedPdf, "UU P  NORM", resLo.data()); //calculate the Chi2Test and store the results in resLo
    ...
    */
    ...
    Char_t str[50];
    sprintf(str, "p-value for Pearson chi2 test: %g", chi2);
    pt3->AddText(str);
    pt3->SetFillColor(18);
    pt3->SetTextFont(20);
    pt3->SetTextColor(4);
    ...
    leg->AddEntry(histo,"Histogram filled with a poisson distribution","l");
...
void testGaussian(){
    ...
    auto histo = new TH1F("h1f","Test random numbers",histo_bins,0,10);
    ...
    TH1F*histo = new TH1F("testhi","Poisson distribution",100,0,5);
    ...
    double sumGaussianHisto=0., sumGaussianHistoScaled=0.;
    ...
    std::cout<<"sumGaussianHisto: "<<sumGaussianHisto<<"\n";
    std::cout<<"sumGaussianHistoScaled: "<<sumGaussianHistoScaled<<"\n";
    ...
    std::cout<<"Alias test with Gaussian: END.\n";
    c1->SaveAs("histo.pdf");
