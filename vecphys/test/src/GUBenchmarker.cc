...
GUBenchmarker::GUBenchmarker()
    : fNtracks(4992), fRepetitions(1), fVerbosity(1), fMinP(defaultMinP), fMaxP(defaultMaxP), fSampleType(kAlias),
  ...
  for (unsigned r = 0; r < fRepetitions; ++r) {
    ...
    for (int k = 0; k < kNumberPhysicsModel; ++k) {
      if (fEmModel == GUPhysicsModelIndex::kNullModel || fEmModel == k) {
        ...
        elapsedT[k] = 0.0;
        elapsedT[k] = ScalarKernelFunc[k](fNtracks, itrack_aos, targetElements, otrack_aos, fSampleType);
  ...
  for (unsigned r = 0; r < fRepetitions; ++r) {
    ...
    for (int k = 0; k < kNumberPhysicsModel; ++k) {
      if (fEmModel == GUPhysicsModelIndex::kNullModel || fEmModel == k) {
        ...
        elapsedT[k] = 0.0;
        elapsedT[k] = G4KernelFunc[k](fNtracks, itrack_aos, targetElements, otrack_aos);
        elapsedTotal[k] += elapsedT[k];
  ...
  for (unsigned r = 0; r < fRepetitions; ++r) {
    ...
    for (int k = 0; k < kNumberPhysicsModel; ++k) {
      if (fEmModel == GUPhysicsModelIndex::kNullModel || fEmModel == k) {
        ...
        elapsedT[k] = 0.0;
        elapsedT[k] = VectorKernelFunc[k](itrack_soa, targetElements, otrack_soa, fSampleType);
        elapsedTotal[k] += elapsedT[k];
...
void GUBenchmarker::PrepareTargetElements(int *targetElements, int ntracks)
{
  ...
  {
    if (report)
      std::cout << " Generating Target Elements with Random mode - mode # = " << fElementMode << " numFx= " << NumFx
                << std::endl;
    ...
    if (report)
      std::cout << " Generating Target Elements cycling through "
                ...
                << " Using " << numElements << " number of elements. " << std::endl;
    for (int i = 0; i < ntracks; ++i) {
      targetElements[i] = ((i * 101 + (noCalls % numElements) * 59) % numElements) + 1;
...
void GUBenchmarker::ReportInteraction(double incomingEnergy, int targetElement, double GammaOut_E, double GammaOut_Pz,
                                      double electron_En, double electron_Pz, bool print_Uz)
{
  ...
  printf(" E_in = %8.5f  elem = %4d E_out = %8.4g  E_elec = %8.4g  Sum(E_out)= %8.4g  Diff(In-Out)= %8.3g   ",
         incomingEnergy, targetElement, GammaOut_E, electron_En, SumEout, incomingEnergy - SumEout);
  if (print_Uz)
