...
void writeMesh(varStore *prm,Int_t geomId,Int_t nv){
  prm->PrimFile << ",\n\t{\"fn\":\"mesh\",\"gid\":"<<geomId<<",\"cdf\":"<<prm->CdF[geomId]<<",\"nv\":"<<nv<<",\"lv\":"<<prm->level <<",\"m\":\""<<getMother(prm) <<"\",\"cl\":" << getColor(prm)  <<",\"vs\":"<<getVisible(prm) <<",\"n\":\"" <<getVolName(prm)<<"\",\"v\":[";
...
void writeCompMesh(varStore *prm,Int_t geomId,TString Comp){
  prm->PrimFile << ",\n\t{\"fn\":\"mesh\",\"gid\":"<<geomId<<",\"cdf\":"<<false<<",\"nv\":0,\"lv\":"<<prm->level <<",\"m\":\"root\",\"cl\":" << getColor(prm)  <<",\"vs\":"<<getVisible(prm) <<",\"n\":\"" <<getVolName(prm) <<Comp<<"\",\"v\":[";
  for (int i=0;i<prm->nM;i++) {
    prm->PrimFile <<prm->Mtrx[i];
    if (i<(prm->nM-1))prm->PrimFile <<",";
  }; 
...
void ExportGeomAssembly(varStore *prm){
  int nv = gGeoManager->CountNodes(prm->current->GetVolume(),prm->MaxVisiLevel-prm->level,1);
  prm->PrimFile << ",\n\t{\"fn\":\"grp\",\"cdf\":"<<prm->CdF[prm->current->GetVolume()->GetNumber()]<<",\"nv\":"<<nv<<",\"lv\":"<<prm->level <<",\"m\":\""<<getMother(prm)<<"\",\"vs\":"<<getVisible(prm)  <<",\"n\":\"" <<getVolName(prm)<<"\",\"v\":[";
  for (int i=0;i<prm->nM;i++) {
    prm->PrimFile <<prm->Mtrx[i];
    if (i<(prm->nM-1))prm->PrimFile <<",";
  }; 
...
void ExportJsonComposite(varStore *prm,TString CompNameL,TString CompNameR,TString operation,TString CompName,Int_t geomId){

  prm->PrimFile << ",\n\t{\"fn\":\"cmp\",\"cdf\":"<<prm->CdF[prm->current->GetVolume()->GetNumber()]<<",\"gid\":"<<geomId<<",\"nv\":"<<gGeoManager->CountNodes(prm->current->GetVolume(),prm->MaxVisiLevel-prm->level,1)<<",\"lv\":"<<prm->level <<",\"m\":\""<<getMother(prm) <<"\",\"cl\":" << getColor(prm)  <<",\"vs\":"<<getVisible(prm) <<",\"v\":[";
  for (int i=0;i<prm->nM;i++) {
    prm->PrimFile <<prm->Mtrx[i];
    if (i<(prm->nM-1))prm->PrimFile <<",";
  }; 
...
void cloneVolume1(varStore *prm,Int_t nv,Int_t geomId){
 prm->ClonesFile << ",\n\t{\"fn\":\"clone\",\"cdf\":"<<prm->CdF[prm->current->GetVolume()->GetNumber()]<<",\"nv\":"<<nv<<",\"lv\":"<<prm->level  <<",\"m\":\""<<getMother(prm) <<"\",\"ovn\":\"" << getVolName(prm) <<"\",\"gid\":"<<prm->current->GetVolume()->GetNumber() <<",\"cn\":"<<prm->current->GetNumber() <<",\"v\":[";
  for (int i=0;i<prm->nM;i++) {
...
void cloneVolume(varStore *prm,Int_t nv,Int_t geomId){
 prm->PrimFile   << ",\n\t{\"fn\":\"clone\",\"cdf\":"<<prm->CdF[prm->current->GetVolume()->GetNumber()]<<",\"nv\":"<<nv<<",\"lv\":"<<prm->level  <<",\"m\":\""<<getMother(prm) <<"\",\"ovn\":\"" << getVolName(prm) <<"\",\"gid\":"<<prm->current->GetVolume()->GetNumber() <<",\"cn\":"<<prm->current->GetNumber() <<",\"v\":[";
