...
void box(Int_t iaxis=0, Int_t ndiv=8, Double_t start=0, Double_t step=0)
{
   ...
   vol->SetLineWidth(2);
   //TGeoRotation   *rot1 = new TGeoRotation("rot1", 90., 0., 90., 0., 0., 0.);
   //top->AddNodeOverlap(vol,1,new TGeoCombiTrans(-57,0,-63,new TGeoRotation("vol",0,15,0)));
   top->AddNodeOverlap(vol,1,new TGeoCombiTrans(0,0,40,new TGeoRotation("vol",0,90,0)));
   ...
   TGeoVolume *top1 = gGeoManager->GetTopVolume();   
   TGeoIterator iter(top1);
...
void para(Int_t iaxis=0, Int_t ndiv=8, Double_t start=0, Double_t step=0)
{
   ...
   TGeoVolume *top = gGeoManager->MakeBox("TOP",med,100,100,100);
   gGeoManager->SetTopVolume(top);
   TGeoVolume *vol = gGeoManager->MakePara("PARA",med, 20,30,40,20,55,65); //30,15,30);
   vol->SetLineColor(randomColor());
   vol->SetLineWidth(2);
   top->AddNode(vol,1);
   if (iaxis) {
...
void sphere(Int_t iaxis=0, Int_t ndiv=8, Double_t start=0, Double_t step=0)
{
   ...
   TGeoVolume *top = gGeoManager->MakeBox("TOP",med,100,100,100);
   gGeoManager->SetTopVolume(top);
   TGeoVolume *vol = gGeoManager->MakeSphere("SPHERE",med, 0,40,0,80,0,120);
   //TGeoVolume *vol =   gGeoManager->MakeSphere("SPHERE",med, 30,40,0,120,30,360);
   vol->SetLineColor(randomColor());
   vol->SetLineWidth(2);
   top->AddNode(vol,1);
   if (iaxis) {
...
void torus(Int_t iaxis=0, Int_t ndiv=8, Double_t start=0, Double_t step=0)
{
   ...
   TGeoVolume *top = gGeoManager->MakeBox("TOP",med,100,100,100);
   gGeoManager->SetTopVolume(top);
   TGeoVolume *vol = gGeoManager->MakeTorus("TORUS",med, 40,20,25,30,200);
   vol->SetLineColor(randomColor());
   top->AddNode(vol,1);
   if (iaxis) {
      TGeoVolume *slice = vol->Divide("SLICE",iaxis,ndiv,start,step);
...
void parab()
{
   ...
   TGeoVolume *top = gGeoManager->MakeBox("TOP",med,100,100,100);
   gGeoManager->SetTopVolume(top);
   TGeoVolume *vol = gGeoManager->MakeParaboloid("PARAB",med,10, 40, 170);
...
void trd2(Int_t iaxis=0, Int_t ndiv=8, Double_t start=0, Double_t step=0)
{
   ...
   vol->SetLineWidth(2);
   //top->AddNode(vol,1);
   top->AddNodeOverlap(vol,1,new TGeoCombiTrans(-57,0,-63,new TGeoRotation("vol",0,90,0)));
   if (iaxis) {
      TGeoVolume *slice = vol->Divide("SLICE",iaxis,ndiv,start,step);
      if (!slice) return;
      slice->SetLineColor(randomColor());
