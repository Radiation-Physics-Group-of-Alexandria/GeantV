...
void ExportJsonComposite(varStore *prm,TString CompNameL,TString CompNameR,TString operation,TString CompName,Int_t geomId){
  ...
  prm->PrimFile << ",\n\t{\"fn\":\"cmp\",\"cdf\":"<<prm->CdF[prm->current->GetVolume()->GetNumber()]<<",\"gid\":"<<geomId<<",\"nv\":"<<gGeoManager->CountNodes(prm->current->GetVolume(),1000,1)<<",\"lv\":"<<prm->level <<",\"m\":\""<<getMother(prm) <<"\",\"cl\":" << getColor(prm)  <<",\"vs\":"<<getVisible(prm) <<",\"v\":[";
  for (int i=0;i<prm->nM;i++) {
    prm->PrimFile <<prm->Mtrx[i];
    if (i<(prm->nM-1))prm->PrimFile <<",";
  }; 
