...
int exportVolGeometry(varStore *prm,TGeoShape *shape,Int_t geomId){
  ...
  TString mat=prm->current->GetVolume()->GetMaterial()->GetName();

  int nvol=gGeoManager->CountNodes(prm->current->GetVolume(),prm->MaxVisiLevel-prm->level,1);

//  if (clsname == "TGeoShapeAssembly" || (( mat == "AIR" || mat == "VACUUM") && (nvol>1) )  ) {
  if (clsname == "TGeoShapeAssembly" ||  mat == "AIR" || mat == "VACUUM" ) {
...
bool checkVol(varStore *prm,Int_t geomId){
   ...
   if (check) prm->LastParent[geomId]=prm->current->GetMotherVolume()->GetNumber();
   prm->MapVol[geomId]=false;
   return check;
}

bool InSameParent(varStore *prm){
  bool csp=prm->LastParent[prm->current->GetVolume()->GetNumber()]==prm->current->GetMotherVolume()->GetNumber();
  prm->LastParent[prm->current->GetVolume()->GetNumber()]=prm->current->GetMotherVolume()->GetNumber();
  return csp;
}


int CheckCloneSameParent(varStore *prm){
  ...
  while ((current = iter.Next())) {
    ...
    if (level <= prm->MaxVisiLevel) {
      if ( !checkVol(prm,vol->GetNumber()) ) {
        if (prm->LastParent[vol->GetNumber()]!=prm->current->GetMotherVolume()->GetNumber()){
         if (prm->LastParent[vol->GetNumber()]==-1) {
            prm->LastParent[vol->GetNumber()]=current->GetMotherVolume()->GetNumber();
          } else {
            if (!prm->CdF[vol->GetNumber()])prm->CdF[vol->GetNumber()]=true;
          }
        }
      }
    } else {
...
int ExportCurrentVolume(varStore *prm,TGeoVolume *vol, TGeoShape *shape,TGeoMatrix *matrix,TString Comp,TString CompL,TString CompR){
  ...
  if (shape->IsComposite()) {
    ...
    Int_t geomId=-1;
    if (Comp=="") geomId=vol->GetNumber();
    ExportJsonComposite(prm,CompNameL,CompNameR,operation,CompName,geomId);

  } else {
    Int_t gId=vol->GetNumber();
    getMatrixVR(prm,matrix);
    if (Comp==""){

        exportVolGeometry(prm,shape,gId);
        ...
        TString clsname=shape->ClassName();
      	if ((clsname != "TGeoShapeAssembly")&& !( mat == "AIR" || mat == "VACUUM") )writeMesh(prm,gId,gGeoManager->CountNodes(vol,prm->MaxVisiLevel-prm->level,1));
    } else {
        prm->nMaxVol+=1;
...
int ExportPhyVols(varStore *prm){
  ...
  while ((current = iter.Next())) {
    ...
    if (prm->level <= prm->MaxVisiLevel) {
      if ( checkVol(prm,vol->GetNumber()) ) {
        ...
        if (mat=="AIR"||mat=="VACUUM") cout<<vol->GetName()<<"\t"<<vol->GetMaterial()->GetName()<<"\t"<<gGeoManager->CountNodes(vol,prm->MaxVisiLevel-prm->level,1)<<"\t"<<shape->ClassName()<<"\t"<<" Ccolor: "<<prm->current->GetColour()<<" Vcolor: "<<prm->current->GetVolume()->GetLineColor( )<<"\n";
        ExportCurrentVolume(prm,vol,shape,current->GetMatrix(),"","","");
        volname=vol->GetName();
      } else {
        getMatrixVR(prm,current->GetMatrix());
        cloneVolume(prm,gGeoManager->CountNodes(vol,prm->MaxVisiLevel-prm->level,1),vol->GetNumber());
        iter.Skip();
      }
  ...
  cout <<"\n"<<gGeoManager->CountNodes(prm->top,prm->MaxVisiLevel,1)<<" Nodes have been defined in the file: "<< prm->PrimFilename<<"\n";
