...
void help() {
  printf("\nUsage: lhcbapp [OPTIONS] INPUT_FILE\n\n");
...
int main(int argc, char *argv[]) {
  ...
  std::string lhcb_geometry_filename("LHCb_201603.root");
  ...
  config->fGeomFileName = lhcb_geometry_filename;
...
#else
//  runMgr->LoadGeometry(lhcb_geometry_filename.c_str());
#endif
#endif
  ...
  LHCbApplication *LHCbApp = new LHCbApplication(runMgr);
  runMgr->SetUserApplication( LHCbApp );
  if (score) {
    LHCbApp->SetScoreType(LHCbApplication::kScore);
  } else {
    LHCbApp->SetScoreType(LHCbApplication::kNoScore);
  }
#ifdef GEANT_TBB
  if (tbbmode)
...
//  propagator->PropagatorGeom(lhcb_geometry_filename.c_str(), n_threads, monitor);
  return 0;
}
