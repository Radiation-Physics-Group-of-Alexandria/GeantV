...
void SetupUserDetector        (userapplication::ClicDetectorConstruction*   detector);
void SetupUserPrimaryGenerator(userapplication::ClicPrimaryGenerator*     primarygun);
void SetupUserApplication     (userapplication::ClicApp*                         app);
void PrintRunInfo();
...
int main(int argc, char *argv[]) {
  ...
  userapplication::UserPhysicsList *userPhysList = new userapplication::UserPhysicsList("ClicPhysicsList");
  SetupUserPhysicsList(userPhysList);
  geantphysics::PhysicsListManager::Instance().RegisterPhysicsList(userPhysList);

  // Create Clic detector construction
  userapplication::ClicDetectorConstruction *calo = new userapplication::ClicDetectorConstruction(runMgr);
  SetupUserDetector(calo);
  runMgr->SetDetectorConstruction(calo);

  // Create Clic primary generator
  userapplication::ClicPrimaryGenerator *caloGun = new userapplication::ClicPrimaryGenerator(calo);
  SetupUserPrimaryGenerator(caloGun);
  runMgr->SetPrimaryGenerator(caloGun);

  // Create the real physics Clic application
  userapplication::ClicApp *caloApplication = new userapplication::ClicApp(runMgr,calo,caloGun);
...
void SetupUserDetector(userapplication::ClicDetectorConstruction* det) {
  if (userLayers) det->SetNumLayers(n_layers);
  if (userAbsorbers) det->SetNumAbsorbers(n_absorbers);
  if (userYZ) det->SetDetectorXY(caloYZ);
...
void SetupUserApplication(userapplication::ClicApp* caloApp) {
