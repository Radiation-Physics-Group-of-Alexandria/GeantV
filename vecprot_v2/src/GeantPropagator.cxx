...
GeantPropagator::GeantPropagator()
    : fNthreads(1), fNevents(100), fNtotal(1000), fNtransported(0), fNprimaries(0), fNsteps(0),
      fNsnext(0), fNphys(0), fNmag(0), fNsmall(0), fNcross(0), fPriorityEvents(0), fDoneEvents(0),
      ...
      fMaxDepth(0), fLearnSteps(0), fLastEvent(0), fPriorityThr(0), fNstepsKillThr(50000), fNminReuse(10000), fNTransportTask(0), fMaxRes(0), fMaxVirt(0), fNaverage(0), fVertex(),
      fEmin(1.E-4), // 100 KeV
      fEmax(10),    // 10 Gev
      // fBmag(0.),
...
void GeantPropagator::InitializeAfterGeom() {
  ...
  if (!fThreadData) {
    ...
    for (int i = 0; i < fNthreads; i++) {
      ...
      // printf("InitializeAfterGeom> Setting field-object for thread %d to %p\n",
      //       i, fThreadData[i]->GetField() );  // fFieldObj );
...
GeantPropagator::ObtainThreadRkPropagator(unsigned int ThreadId) {
  ...
  if( fInitialisedRKIntegration && fUseRungeKutta ){
    ...
    printf("Obtaining RK propagator for %p for thread %d", fieldPropagator, ThreadId);
...
GUVField* GeantPropagator::ObtainField(unsigned int threadId) {
  ...
  if( fInitialisedRKIntegration && fUseRungeKutta ){
    ...
    pField = fThreadData[threadId]->fFieldObj;
    if( ! pField ) {
      if( fUserDetectorCtion ) {
        GUVField* masterFld= fUserDetectorCtion->GetField();
        if( masterFld ){
          printf("GeantPropagator::ObtainField> Creating field Object for thread %d\n", threadId);
          pField= masterFld->Clone();  // CloneOrSafeSelf();
        }
        printf(" Created field Object = %p for thread %d\n", pField, threadId);
        fThreadData[threadId]->fFieldObj= pField;
      }
    }
