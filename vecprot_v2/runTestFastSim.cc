...
void SetupUserPhysicsListFS     (userfsapplication::UserPhysicsListFS*            physlist);
void SetupUserDetector        (userfsapplication::UserDetectorConstructionFS*   detector);
void SetupUserPrimaryGeneratorFS(userfsapplication::UserPrimaryGeneratorFS*     primarygun);
void SetupUserApplication     (userfsapplication::TestFastSim*                         app);
...
int main(int argc, char *argv[]) {
  ...
  userfsapplication::UserPhysicsListFS *userPhysList         = new userfsapplication::UserPhysicsListFS("testFastSim-user-physics");
  SetupUserPhysicsListFS(userPhysList);
  ...
  userfsapplication::UserDetectorConstructionFS *detector    = new userfsapplication::UserDetectorConstructionFS(runManager);
  ...
  userfsapplication::UserPrimaryGeneratorFS *primaryGenerator = new userfsapplication::UserPrimaryGeneratorFS(detector);
  SetupUserPrimaryGeneratorFS(primaryGenerator);
  runManager->SetPrimaryGenerator(primaryGenerator);
  //
  // create the testFastSim user application object, set its configurable parameters and register in the GeantRunManager
  userfsapplication::TestFastSim *testFastSimApplication            = new userfsapplication::TestFastSim(runManager, detector, primaryGenerator);
  SetupUserApplication(testFastSimApplication);
  runManager->SetUserApplication (testFastSimApplication);
  //
  // run the simulation
...
//
// TestFastSim application parameters
...
void help() {
  printf("\nUsage: runTestFastSim [OPTIONS] INPUT_FILE\n\n");
...
void SetupUserPhysicsListFS(userfsapplication::UserPhysicsListFS* physlist) {
  if (parProcessMSCStepLimit!="") {
    if (parProcessMSCStepLimit=="UseSafety") {
      ...
      std::cerr<< " **** ERROR runTestFastSim::SetupUserPhysicsListFS() \n"
