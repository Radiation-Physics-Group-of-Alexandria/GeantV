...
// @author S. Vallecorsa, derived from examples by M Novak and R Schmitz
// @date Septemebrs 1, 2017
//
//     //===----------------------------------------------------------------------===//
//
#include "ClicApp.h"
...
ClicApp::ClicApp(Geant::GeantRunManager *runmgr, ClicDetectorConstruction *det, ClicPrimaryGenerator *gun)
  : Geant::GeantVApplication(runmgr), fDetector(det), fPrimaryGun(gun) {
  fHist1FileName         = "ClicAppHist";
  fInitialized           = false;
...
void ClicApp::AttachUserData(Geant::GeantTaskData *td) {
  ...
  ClicAppThreadDataEvents *eventData = new ClicAppThreadDataEvents(fNumBufferedEvents, fNumPrimaryPerEvent);
  ...
  ClicAppThreadDataRun *runData = new ClicAppThreadDataRun();
...
bool ClicApp::Initialize() {
  ...
  if (!fDetector) {
    Geant::Error("ClicApp::Initialize", "Geometry not available!");
    return false;
  }
  ...
  if (!fPrimaryGun) {
    Geant::Error("ClicApp::Initialize", "PrimaryGenerator not available!");
    return false;
  }
  ...
  fDataHandlerEvents = fRunMgr->GetTDManager()->RegisterUserData<ClicAppThreadDataEvents>("ClicAppThreadDataEvents");
  fDataHandlerRun    = fRunMgr->GetTDManager()->RegisterUserData<ClicAppThreadDataRun>("ClicAppThreadDataRun");
  //
  // create the unique, global data struture that will be used to store cumulated per-primary data during the simulation
  fData        = new ClicAppData();
  //
...
void ClicApp::SteppingActions(Geant::GeantTrack &track, Geant::GeantTaskData *td) {
  ...
  ClicAppDataPerPrimary &dataPerPrimary =  (*fDataHandlerEvents)(td).GetDataPerEvent(track.fEvslot).GetDataPerPrimary(track.PrimaryParticleIndex());
  ...
  if (validVolume) {
    ...
    if (isTransmit && isPrimary) {
          ...
          ClicAppThreadDataRun  &dataRun =  (*fDataHandlerRun)(td);
...
void ClicApp::Digitize(Geant::GeantEvent *event) {
  ...
  ClicAppThreadDataEvents *data = fRunMgr->GetTDManager()->MergeUserData(event->GetSlot(), *fDataHandlerEvents);
  ...
  ClicAppDataPerEvent &dataPerEvent = data->GetDataPerEvent(event->GetSlot());
  fMutex.lock();
...
void ClicApp::FinishRun() {
  ...
  ClicAppThreadDataRun *runData = fRunMgr->GetTDManager()->MergeUserData(-1, *fDataHandlerRun);
  //
