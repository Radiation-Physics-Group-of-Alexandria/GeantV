...
#include "UserDetectorConstructionFS.h"
#include "UserPrimaryGeneratorFS.h"
...
TestFastSim::TestFastSim(Geant::GeantRunManager *runmgr, UserDetectorConstructionFS *det, UserPrimaryGeneratorFS *gun)
  : Geant::GeantVApplication(runmgr), fDetector(det), fPrimaryGun(gun) {
  fHist1FileName         = "testFastSimHist1.dat";
  fInitialized           = false;
...
void TestFastSim::AttachUserData(Geant::GeantTaskData *td) {
  ...
  TestFastSimThreadDataEvents *eventData = new TestFastSimThreadDataEvents(fNumBufferedEvents, fNumPrimaryPerEvent);
  ...
  TestFastSimThreadDataRun *runData = new TestFastSimThreadDataRun();
...
bool TestFastSim::Initialize() {
  ...
  if (!fDetector) {
    Geant::Error("TestFastSim::Initialize", "Geometry not available!");
    return false;
  }
  ...
  if (!fPrimaryGun) {
    Geant::Error("TestFastSim::Initialize", "PrimaryGenertaor not available!");
    return false;
  }
  ...
  fDataHandlerEvents = fRunMgr->GetTDManager()->RegisterUserData<TestFastSimThreadDataEvents>("TestFastSimThreadDataEvents");
  fDataHandlerRun    = fRunMgr->GetTDManager()->RegisterUserData<TestFastSimThreadDataRun>("TestFastSimThreadDataRun");
  //
  // create the unique, global data struture that will be used to store cumulated per-primary data during the simulation
  fData        = new TestFastSimData();
  //
...
void TestFastSim::SteppingActions(Geant::GeantTrack &track, Geant::GeantTaskData *td) {
  ...
  TestFastSimDataPerPrimary &dataPerPrimary =  (*fDataHandlerEvents)(td).GetDataPerEvent(track.fEvslot).GetDataPerPrimary(track.PrimaryParticleIndex());
    ...
    if (isTransmit && charge!=0.) {
      ...
      if (cost>0.0) {
        ...
        if (theta>=fHist1Min && theta<fHist1Max) {
          ...
          TestFastSimThreadDataRun  &dataRun =  (*fDataHandlerRun)(td);
...
void TestFastSim::Digitize(Geant::GeantEvent *event) {
  ...
  TestFastSimThreadDataEvents *data = fRunMgr->GetTDManager()->MergeUserData(event->GetSlot(), *fDataHandlerEvents);
  ...
  TestFastSimDataPerEvent &dataPerEvent = data->GetDataPerEvent(event->GetSlot());
  fMutex.lock();
...
void TestFastSim::FinishRun() {
  ...
  TestFastSimThreadDataRun *runData = fRunMgr->GetTDManager()->MergeUserData(-1, *fDataHandlerRun);
  //
  ...
  if (pManager->HasEnergyLossProcess()) {
    ...
    for (size_t ip=0; ip<procVect.size(); ++ip) {
      if (procVect[ip]->GetType()==geantphysics::ProcessType::kEnergyLoss) {
        ...
        dEdXRestrComputed += emProc->ComputeDEDX(targetMatCut, fPrimaryGun->GetPrimaryParticleEnergy(), fPrimaryGun->GetPrimaryParticle());
        dEdXFullComputed  += emProc->ComputeDEDX(targetMatCut, fPrimaryGun->GetPrimaryParticleEnergy(), fPrimaryGun->GetPrimaryParticle(),true);
      }
    }
  }
