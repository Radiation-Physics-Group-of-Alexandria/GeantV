...
/**
 * @brief   User defined data structures for the TestFastSim GeantV application to store/handle scoring data.
 * @author  M Novak
 * @date    July 2017
 *
 ...
 * An additional, global data structure, TestFastSimData is defined in this application to accumulate data that belongs to
...
// Data structure per-primary particle for TestFastSim
class TestFastSimDataPerPrimary {
public:
  TestFastSimDataPerPrimary();
 ~TestFastSimDataPerPrimary() { /*nothing to do*/}
  ...
  void   SetOneTransmitted(double val) { if (!fNumOneTrans) fNumOneTrans = val;   }
  double GetOneTransmitted() const     { return fNumOneTrans; }

  void   SetOneReflected(double val)   { if (!fNumOneRefl) fNumOneRefl = val;   }
  ...
  void   Clear();
  TestFastSimDataPerPrimary& operator+=(const TestFastSimDataPerPrimary& other);
...
class TestFastSimData {
public:
  TestFastSimData();
 ~TestFastSimData() { /*nothing to do*/ }
 ...
 void   AddOneTransmitted(double val)       { if (val>0.) fNumOneTrans += 1; }
 double GetOneTransmitted()           const { return fNumOneTrans; }

 void   AddOneReflected(double val)         { if (val>0.) fNumOneRefl += 1.;  }
 double GetOneReflected()             const { return fNumOneRefl;  }

 void   AddEdepInTarget(double val)       { fEdepInTarget += val; fEdepInTarget2 += val*val; }
 double GetEdepInTarget()           const { return fEdepInTarget;  }
 ...
 void   AddDataPerPrimary(TestFastSimDataPerPrimary& data);
...
// Data structure per-event for TeatFastSim(contain as many per-primary data structures as number of primaries in one event)
class TestFastSimDataPerEvent {
public:
  TestFastSimDataPerEvent(int nprimperevent);
 ~TestFastSimDataPerEvent() {/*nothing to do*/}

  int GetNumberOfPrimaryPerEvent() const { return fNumPrimaryPerEvent; }
  void Clear();
  TestFastSimDataPerPrimary& GetDataPerPrimary(int primindx) { return fPerPrimaryData[primindx]; }
  TestFastSimDataPerEvent& operator+=(const TestFastSimDataPerEvent& other);

private:
  int fNumPrimaryPerEvent;
  std::vector<TestFastSimDataPerPrimary>  fPerPrimaryData; // as many as primary particle in an event
};



// Thread local data structure for TestFastSim to collecet/handle thread local multiple per-event data structures (as many
...
class TestFastSimThreadDataEvents {
public:
  TestFastSimThreadDataEvents(int nevtbuffered, int nprimperevent);
 ~TestFastSimThreadDataEvents() {/*nothing to do*/}

  void  Clear(int evtslotindx) { fPerEventData[evtslotindx].Clear(); }
//  void  Clear() {
//    for (int i=0; i<fNumBufferedEvents; ++i) Clear(i);
//  }
  bool  Merge(int evtslotindx, const TestFastSimThreadDataEvents &other);

  TestFastSimDataPerEvent& GetDataPerEvent(int evtslotindx) { return fPerEventData[evtslotindx]; }
  const TestFastSimDataPerEvent& GetDataPerEvent(int evtslotindx) const { return fPerEventData[evtslotindx]; }

private:
  int                                fNumBufferedEvents;
  std::vector<TestFastSimDataPerEvent>   fPerEventData;
};


// Thread local data structure for TestFastSim to collecet/handle thread local run-global data structures. The user defined
...
class TestFastSimThreadDataRun {
public:
 ...
 bool   Merge(int /*evtslotindx*/, const TestFastSimThreadDataRun& other);
