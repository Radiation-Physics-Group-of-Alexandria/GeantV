...
#ifndef TESTFASTSIM_H
#define TESTFASTSIM_H
...
/**
 * @brief GeantV implementation of the Geant4 TestFastSim application.
 *
...
/** @brief TestFastSim user application */
class TestFastSim : public Geant::GeantVApplication {
public:

  /** @brief Constructor TestFastSim */
  TestFastSim(Geant::GeantRunManager *runmgr, UserDetectorConstructionFS *det, UserPrimaryGeneratorFS *gun);

  /** @brief Destructor TestFastSim */
  virtual ~TestFastSim();
...
private:
  /** @brief Copy constructor TestFastSim (deleted) */
  TestFastSim(const TestFastSim &) = delete;
  /** @brief Operator= for TestFastSim (deleted) */
  TestFastSim &operator=(const TestFastSim &) = delete;
...
private:
  ...
  // this data will be obtained from the UserDetectorConstructionFS at initialization
  ...
  Geant::TaskDataHandle<TestFastSimThreadDataEvents>  *fDataHandlerEvents;
  // 2. merged from all working threads when transportation of all events (i.e. end of the simulation) are completed
  Geant::TaskDataHandle<TestFastSimThreadDataRun>     *fDataHandlerRun;
  // a unique, run-global user defined data structure to store cumulated quantities per primary particle during the simulation
  TestFastSimData  *fData;
  //
  UserDetectorConstructionFS      *fDetector;
  UserPrimaryGeneratorFS          *fPrimaryGun;
  // mutex to prevent multiple threads writing into the unique, run-global TestFastSimData object (in the Digitization after
